...
/* ==========================================
   URL DE GOOGLE SHEETS PARA EXCEL
   ========================================== */
const EXCEL_SHEET = 'https://opensheet.elk.sh/1iJuUsML7AqVkmk_j93dKmdLg5p6a5cYPhJ25kN7Un-s/Hoja1';
...
/* ==========================================
   ...
   ========================================== */
const DRIVE_IMAGES_SHEET = 'https://opensheet.elk.sh/2PACX-1vT1oyGRpf2YGthO4yA9Mg1vRozEQwv2zeSDQAmObdWPnNUrCkgJgJDWvXo3vivkkqVwRF2z78mJXEQC/Hoja1';
...
function renderExcel() {
    panelTitle.innerText = '📊 Datos desde Excel';
    
    panelContent.innerHTML = `<p class="loading-text">Cargando datos...</p>`;
    ...
    fetch(EXCEL_SHEET)
        ...
        .then(data => {
            if (!data || data.length === 0) {
                panelContent.innerHTML = '<p class="load-error">No hay datos en el Excel.</p>';
                return;
            }

            let html = `<div class="contact-list">`;

            // Generar tarjeta para cada fila del Excel
            data.forEach(c => {
                ...
                html += `
                    <div class="contact-item">
                        ...
                        <div class="contact-phone">${numero}</div>
                        ${numero ? `<a href="https://wa.me/${numero}" target="_blank" class="btn-whatsapp-excel">WhatsApp</a>` : ''}
                    </div>
                `;
            });
        ...
        .catch(err => {
            panelContent.innerHTML = `<p class="load-error">Error cargando datos. Verifique que el sheet esté publicado.</p>`;
            console.error(err);
        });
}
...
function renderGuia() {
    ...
    const guideHtml = `
        ...
        <div class="guide-step">
            ...
            <p>Carga datos desde Google Sheets externo. Ideal para mantener contactos actualizados sin modificar el código.</p>
        </div>
        <div class="guide-step">
